...
| Interface ID | Source | Objet | Contrat | Version | Destination | Point-in-time | Provenance | Impact 04 | Failure mode |
|---|---|---|---|---|---|---|---|---|---|
| IF-001 | Données / contrôle qualité | Qualité et continuité | Principe rappelé par `04 §9`, détail `05 §1–2` | 04 v0.6.1 / 05 v0.1 | Validation | À définir | Rapport de couverture | Bloquant pour conditions concernées | Série discontinue ou couverture incomplète |
| IF-002 | Dataset validé | Coûts complets | `04 §5.3 c.4` + `05 §7` | 04 v0.6.1 / 05 v0.1 | Validation | Historique de marché | Modèle de coûts | Bloquant | Coûts absents / incohérents |
| IF-003 | Pipeline technique | Gel technique | `04 §3.4` + `05` ou document technique | Partiellement défini | Recherche | À définir | Journal de configuration | Bloquant selon contrôle | Configuration modifiée / non traçable |
| IF-004 | Asset Profile | Capacité / liquidité / impact / exposition / levier | Explicitement hors `04`, relève de `05` ou risque | Non défini | Validation / risque | Historique + contexte | Asset Profile / Risk | Potentiellement bloquant | Contrainte absente |
| IF-005 | Dataset transformé | Identité + limites d'usage | `05 §3` | 0.1 proposition | Moteur | À définir | Provenance parent | Potentiellement bloquant | Transformation non déclarée |
| IF-006 | Moteur A | Moteur B | Parité d'implémentation | 0.1 proposition | Moteur de référence / portage | Même données | Rapport de parité | Condition de confiance | Divergence inexpliquée |
...
La conservation de l'historique est obligatoire comme exigence d'architecture cible, mais cette section ne constitue pas encore un contrat normatif.
