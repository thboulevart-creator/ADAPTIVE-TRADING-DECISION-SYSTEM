...
RUNTIME_PATH = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch05_runtime.json"
ADJUDICATION_JSON = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch05_adjudication.json"
QUALIFICATION_MD = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch05_qualification.md"

EXPECTED_RUN = 34947146056
EXPECTED_JOB = 104309150262
EXPECTED_ARTIFACT = 10386998786
EXPECTED_ARTIFACT_SHA256 = "ad96e1850ca53910c092abd444f02a04e2a84ea192fa6c0b5189a7e349ea800c"
EXPECTED_PROBE_COMMIT = "33ae476c48372bce64421a411066db2ddea6125c"
...
def _validate_runtime_identity(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if runtime.get("batch_number") != 5 or runtime.get("batch_size") != BATCH_SIZE:
    ...
    expected_targets = [{"date": day.isoformat(), "reason": reason} for day, reason in batch05_targets()]
...
def _validate_result_envelope(raw: dict[str, Any], target_day: date, candidate_reason: str) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
...
def _overlap_blocked_verdict(*, target_day: date, candidate_reason: str, network_record: dict[str, Any], dom_record: dict[str, Any] | None, provenance: dict[str, Any]) -> dict[str, Any]:
...
def adjudicate_runtime(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_targets = batch05_targets()
    ...
    for raw, (target_day, candidate_reason) in zip(results, expected_targets, strict=True):
        ...
        if network_record is not None:
            ...
            if start is not None and start.date() != target_day:
                verdict = _overlap_blocked_verdict(target_day=target_day, candidate_reason=candidate_reason, network_record=network_record, dom_record=dom_record, provenance=provenance)
            else:
                evidence = RecoveryEvidence(target_date=target_day, candidate_reason=candidate_reason, requested_date=target_day, instrument_id=str(raw["instrument_id_observed"]), instrument_name=str(raw["instrument_name"]), network_record=network_record, raw_payload_present=True, dom_available=dom_record is not None, dom_record=dom_record, workflow_run=provenance["workflow_run"], artifact_id=provenance["artifact_id"], artifact_sha256=provenance["artifact_sha256"], probe_commit=provenance["probe_commit"])
                verdict = validate_positive_recovery_against_frozen_batch(evidence, expected_targets)
        else:
            evidence = RecoveryEvidence(target_date=target_day, candidate_reason=candidate_reason, requested_date=target_day, instrument_id=str(raw["instrument_id_observed"]), instrument_name=str(raw["instrument_name"]), network_record=None, raw_payload_present=True, dom_available=dom_record is not None, dom_record=dom_record, workflow_run=provenance["workflow_run"], artifact_id=provenance["artifact_id"], artifact_sha256=provenance["artifact_sha256"], probe_commit=provenance["probe_commit"])
            verdict = validate_positive_recovery_against_frozen_batch(evidence, expected_targets)
    ...
    return {
        "schema": "HISTORICAL_TRADING_BREAKS_RECOVERY_BATCH05_ADJUDICATION_V1",
        "batch_contract": BATCH_CONTRACT,
        "parent_protocol": RECOVERY_PROTOCOL,
        "verdict": overall,
        "reason": "BATCH05_POSITIVE_RECORDS_INDEPENDENTLY_ADJUDICATED_WITH_NO_CROSS_DATE_PROMOTION" if overall == "PASS" else "BATCH05_ADJUDICATION_CONTAINS_FAIL",
...
def _render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# HISTORICAL TRADING BREAKS RECOVERY — BATCH 05 QUALIFICATION",
        "",
        ...
        "- replay scope: immutable frozen Batch 05 membership",
        "",
        "## Independent date-level adjudication",
        "",
    ]
    for item, (day, reason) in zip(report["adjudications"], batch05_targets(), strict=True):
        lines += [f"### {day.isoformat()} — {reason}", "", f"**{item['verdict']} — `{item['reason']}`**", ""]
    ...
    lines += ["## Final accounting", "", f"- attempted: `{report['attempted']}`", f"- PASS: `{report['pass']}`", f"- BLOCKED: `{report['blocked']}`", f"- FAIL: `{report['fail']}`", "", "No calendar integration is performed by this adjudicator.", ""]
    return "\n".join(lines)
