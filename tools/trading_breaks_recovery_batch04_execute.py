...
OUT = Path("trading_breaks_batch04_artifacts")
...
async def main() -> int:
    ...
    # Batch 04 execution identity comes only from the historically frozen tuple.
    # It is never recalculated from the live eligible/recovery queue here.
    capture.OUT = OUT
    OUT.mkdir(parents=True, exist_ok=True)

    targets = batch04_targets()
    if len(targets) != BATCH_SIZE:
        raise RuntimeError("FROZEN_BATCH04_SIZE_DRIFT")
