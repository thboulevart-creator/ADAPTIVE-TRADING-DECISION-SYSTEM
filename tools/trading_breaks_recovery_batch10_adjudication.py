...
RUNTIME_PATH = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch10_runtime.json"
ADJUDICATION_JSON = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch10_adjudication.json"
QUALIFICATION_MD = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch10_qualification.md"

EXPECTED_RUN = 35004172846
EXPECTED_JOB = 104499660140
EXPECTED_ARTIFACT = 10411022092
EXPECTED_ARTIFACT_SHA256 = "1572cc5a1c38998f59d32e107b1bcb006a74b288fe32c70ffe019726b3ad5f14"
EXPECTED_PROBE_COMMIT = "443b3696e4e2740a54354787de231c886f90b26e"
EXPECTED_ARTIFACT_URL = (
    "https://github.com/thboulevart-creator/ADAPTIVE-TRADING-DECISION-SYSTEM/"
    "actions/runs/35004172846/artifacts/10411022092"
)
TARGET_INSTRUMENT_NAME = "USATECH.IDX/USD"
TARGET_INSTRUMENT_ID = "9016"
ADJUDICATION_SCHEMA = "HISTORICAL_TRADING_BREAKS_RECOVERY_BATCH10_ADJUDICATION_V1"
...
def _validate_runtime_identity(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if runtime.get("batch_number") != 10 or runtime.get("batch_size") != BATCH_SIZE:
    ...
    expected_targets = [
        ...
        for target_day, reason in batch10_targets()
    ]
...
def adjudicate_runtime(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_targets = batch10_targets()
    ...
    return {
        ...
        "verdict": overall,
        "reason": (
            "BATCH10_POSITIVE_RECORDS_INDEPENDENTLY_ADJUDICATED_WITH_NO_CROSS_DATE_OR_MISSING_DOM_PROMOTION"
            if overall == "PASS"
            else "BATCH10_ADJUDICATION_CONTAINS_FAIL"
        ),
    ...
    for item, (target_day, reason) in zip(
        ...
        if item["verdict"] == "PASS":
            ...
            lines += [
                ...
                f"- DOM witness: `{item.get('dom_witness_present')}`",
