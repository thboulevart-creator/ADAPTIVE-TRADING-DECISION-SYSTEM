...
def rewrite_current_state_tests() -> None:
    replacements = {
        ...
        'sum(not d.eligible and d.latest_attempt_outcome == "BLOCKED" for d in decisions) == 13': 'sum(not d.eligible and d.latest_attempt_outcome == "BLOCKED" for d in decisions) == 14',
        'sum(not item.eligible and item.latest_attempt_outcome == "BLOCKED" for item in decisions) == 13': 'sum(not item.eligible and item.latest_attempt_outcome == "BLOCKED" for item in decisions) == 14',
        "sum(not d.eligible and d.latest_attempt_outcome == 'BLOCKED' for d in decisions) == 13": "sum(not d.eligible and d.latest_attempt_outcome == 'BLOCKED' for d in decisions) == 14",
        "sum(not item.eligible and item.latest_attempt_outcome == 'BLOCKED' for item in decisions) == 13": "sum(not item.eligible and item.latest_attempt_outcome == 'BLOCKED' for item in decisions) == 14",
        '(111, 65, 46)': '(111, 69, 42)',
        '(68, 42, 26)': '(68, 46, 22)',
