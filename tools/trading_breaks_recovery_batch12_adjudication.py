...
RUNTIME_PATH = REPO / 'reports' / 'data-qualification' / 'historical_trading_breaks_recovery_batch12_runtime.json'
ADJUDICATION_JSON = REPO / 'reports' / 'data-qualification' / 'historical_trading_breaks_recovery_batch12_adjudication.json'
QUALIFICATION_MD = REPO / 'reports' / 'data-qualification' / 'historical_trading_breaks_recovery_batch12_qualification.md'

EXPECTED_RUN = 35016454761
EXPECTED_JOB = 104540999314
EXPECTED_ARTIFACT = 10416410006
EXPECTED_ARTIFACT_SHA256 = '6649d976bb9d586cce591cd9b9a9e0e71ed8e5496a1e47e2e52bbdaa2de0297d'
EXPECTED_PROBE_COMMIT = '2c2fd6e2db2e0ab75a6b978d6cddad679dbda5b8'
EXPECTED_ARTIFACT_URL = (
    'https://github.com/thboulevart-creator/ADAPTIVE-TRADING-DECISION-SYSTEM/'
    'actions/runs/35016454761/artifacts/10416410006'
)
TARGET_INSTRUMENT_NAME = 'USATECH.IDX/USD'
TARGET_INSTRUMENT_ID = '9016'
ADJUDICATION_SCHEMA = 'HISTORICAL_TRADING_BREAKS_RECOVERY_BATCH12_ADJUDICATION_V1'
...
def _validate_runtime_identity(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if runtime.get('batch_number') != 12 or runtime.get('batch_size') != BATCH_SIZE:
    ...
    expected_targets = [
        ...
        for target_day, reason in batch12_targets()
    ]
...
def _validate_result_envelope(
    ...
    target_midnight = datetime(target_day.year, target_day.month, target_day.day, tzinfo=timezone.utc)
...
def adjudicate_runtime(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_targets = batch12_targets()
    ...
    for raw, (target_day, candidate_reason) in zip(results, expected_targets, strict=True):
        ...
        adjudications.append(
            _adjudicate_one(raw, target_day, candidate_reason, provenance, expected_targets)
        )
    ...
    return {
        ...
        'verdict': overall,
        'reason': (
            'BATCH12_POSITIVE_RECORDS_INDEPENDENTLY_ADJUDICATED_WITH_NO_CROSS_DATE_PROMOTION'
            if overall == 'PASS'
            else 'BATCH12_ADJUDICATION_CONTAINS_FAIL'
        ),
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    for item, (target_day, reason) in zip(report['adjudications'], batch12_targets(), strict=True):
        lines += [
