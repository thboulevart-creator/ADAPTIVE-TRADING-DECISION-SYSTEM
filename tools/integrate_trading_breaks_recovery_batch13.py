...
def rewrite_current_state_tests() -> None:
    replacements = {
        ...
        'sum((not item.eligible) and item.latest_attempt_outcome == "BLOCKED" for item in decisions) == 14': 'sum((not item.eligible) and item.latest_attempt_outcome == "BLOCKED" for item in decisions) == 16',
        'sum((not d.eligible) and d.latest_attempt_outcome == "BLOCKED" for d in decisions) == 14': 'sum((not d.eligible) and d.latest_attempt_outcome == "BLOCKED" for d in decisions) == 16',
        "sum((not item.eligible) and item.latest_attempt_outcome == 'BLOCKED' for item in decisions) == 14": "sum((not item.eligible) and item.latest_attempt_outcome == 'BLOCKED' for item in decisions) == 16",
        "sum((not d.eligible) and d.latest_attempt_outcome == 'BLOCKED' for d in decisions) == 14": "sum((not d.eligible) and d.latest_attempt_outcome == 'BLOCKED' for d in decisions) == 16",
        'sum(not item.eligible and item.latest_attempt_outcome == "BLOCKED" for item in decisions) == 14': 'sum(not item.eligible and item.latest_attempt_outcome == "BLOCKED" for item in decisions) == 16',
        'sum(not d.eligible and d.latest_attempt_outcome == "BLOCKED" for d in decisions) == 14': 'sum(not d.eligible and d.latest_attempt_outcome == "BLOCKED" for d in decisions) == 16',
