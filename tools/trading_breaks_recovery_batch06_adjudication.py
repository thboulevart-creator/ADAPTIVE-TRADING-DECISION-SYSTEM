...
RUNTIME_PATH = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch06_runtime.json"
ADJUDICATION_JSON = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch06_adjudication.json"
QUALIFICATION_MD = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch06_qualification.md"

EXPECTED_RUN = 34954308324
EXPECTED_JOB = 104332522379
EXPECTED_ARTIFACT = 10390926878
EXPECTED_ARTIFACT_SHA256 = "1e21a4fac890059f436c1488407b5f8ca92809dda18aa220e6af41ee6dfa1052"
EXPECTED_PROBE_COMMIT = "e968db2be1fbfd4d2c419f9dad717ca479b52edd"
...
def _validate_runtime_identity(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if runtime.get("batch_number") != 6 or runtime.get("batch_size") != BATCH_SIZE:
    ...
    expected_targets = [
        ...
        for day, reason in batch06_targets()
    ]
...
def adjudicate_runtime(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_targets = batch06_targets()
    ...
    return {
        "schema": "HISTORICAL_TRADING_BREAKS_RECOVERY_BATCH06_ADJUDICATION_V1",
        "batch_contract": BATCH_CONTRACT,
        "parent_protocol": RECOVERY_PROTOCOL,
        "verdict": overall,
        "reason": (
            "BATCH06_POSITIVE_RECORDS_INDEPENDENTLY_ADJUDICATED_WITH_NO_CROSS_DATE_PROMOTION"
            if overall == "PASS"
            else "BATCH06_ADJUDICATION_CONTAINS_FAIL"
        ),
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    for item, (day, reason) in zip(
        report["adjudications"], batch06_targets(), strict=True
    ):
        lines += [
