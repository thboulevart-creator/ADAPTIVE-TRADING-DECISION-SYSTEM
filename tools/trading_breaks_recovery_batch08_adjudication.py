...
RUNTIME_PATH = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch08_runtime.json"
ADJUDICATION_JSON = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch08_adjudication.json"
QUALIFICATION_MD = REPO / "reports" / "data-qualification" / "historical_trading_breaks_recovery_batch08_qualification.md"

EXPECTED_RUN = 34984538763
EXPECTED_JOB = 104433139005
EXPECTED_ARTIFACT = 10402433119
EXPECTED_ARTIFACT_SHA256 = "644e6d6776792dac03e7cb87a3bd63af0be603c6efe951f44c8911ecd9defadd"
EXPECTED_PROBE_COMMIT = "5cc4834af2c75de99f6e3427f31ab07b38b42611"
...
def _validate_runtime_identity(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if runtime.get("batch_number") != 8 or runtime.get("batch_size") != BATCH_SIZE:
    ...
    expected_targets = [{"date": d.isoformat(), "reason": r} for d, r in batch08_targets()]
...
def adjudicate_runtime(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_targets = batch08_targets()
    ...
    return {
        "schema": "HISTORICAL_TRADING_BREAKS_RECOVERY_BATCH08_ADJUDICATION_V1",
        "batch_contract": BATCH_CONTRACT,
        "parent_protocol": RECOVERY_PROTOCOL,
        "verdict": overall,
        "reason": "BATCH08_POSITIVE_RECORDS_INDEPENDENTLY_ADJUDICATED_WITH_NO_CROSS_DATE_PROMOTION" if overall == "PASS" else "BATCH08_ADJUDICATION_CONTAINS_FAIL",
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    for item, (day, reason) in zip(report["adjudications"], batch08_targets(), strict=True):
...
def main() -> int:
    ...
    QUALIFICATION_MD.write_text(_render_markdown(report) + "\n", encoding="utf-8")
