...
import tools.trading_breaks_recovery_batch10_execute as execution
from tools.trading_breaks_recovery_batch10 import BATCH_SIZE, batch10_targets
...
def test_execution_surface_consumes_only_frozen_batch10_membership():
    ...
    assert "derive_batch10_membership" not in calls
...
def test_shortened_membership_fails_before_any_probe(monkeypatch):
    ...
    monkeypatch.setattr(execution, "batch10_targets", lambda: expected[:-1])
    monkeypatch.setattr(execution.capture, "probe_candidate", fake_probe)
    with pytest.raises(RuntimeError, match="FROZEN_BATCH10_SIZE_DRIFT"):
...
def test_reordered_membership_fails_before_any_probe(monkeypatch):
    ...
    monkeypatch.setattr(execution, "batch10_targets", lambda: attacked)
    monkeypatch.setattr(execution.capture, "probe_candidate", fake_probe)
    with pytest.raises(RuntimeError, match="FROZEN_BATCH10_ORDER_DRIFT"):
        asyncio.run(execution.execute_frozen_batch(object()))
    assert calls == 0
...
def test_duplicate_membership_fails_before_any_probe(monkeypatch):
    ...
    monkeypatch.setattr(execution, "batch10_targets", lambda: attacked)
    monkeypatch.setattr(execution.capture, "probe_candidate", fake_probe)
    with pytest.raises(RuntimeError, match="FROZEN_BATCH10_DUPLICATE_TARGET"):
        asyncio.run(execution.execute_frozen_batch(object()))
    assert calls == 0
