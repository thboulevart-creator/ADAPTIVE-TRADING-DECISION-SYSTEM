...
def test_attempt_ledger_preserves_all_thirty_five_historical_attempts_and_duplicate_history():
    ...
    assert [item.attempt_sequence for item in attempts] == list(range(1, 36))
    assert len({item.attempt_id for item in attempts}) == 35
...
def test_progression_plan_covers_every_unresolved_candidate_without_hidden_skipping():
    ...
    assert len(decisions) == len(queue) == 42
